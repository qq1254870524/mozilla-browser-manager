...
title Mozilla Browser Manager - Desktop Client
...
echo ============================================
echo  Mozilla Browser Manager - Desktop Client
echo  ROOT=%CD%
echo ============================================
...
python -c "import fastapi,uvicorn,playwright" 1>nul 2>nul
...
dir /b "runtime\browsers\chromium*" 1>nul 2>nul
if errorlevel 1 (
  echo [3/4] First-run: install Playwright Chromium to runtime\browsers ...
  python -m playwright install chromium
) else (
  echo [3/4] Chromium runtime present
)

echo [4/4] Starting desktop client ...
echo Close the window = stop API + browsers + mihomo + background jobs
echo.
